...
export type MediaType = 'video' | 'audio' | 'hls' | 'dash' | 'ts' | 'stream' | 'file';
...
export function classify(url: string, contentType: string): Classification {
  ...
  if (ext !== null && FILE_EXTS.has(ext)) return { isMedia: false, type: 'file', ext };
...
export function cleanBiliTitle(t: string): string {
...
export function applyCapture(state: CaptureState, cap: Capture): CaptureResult {
  ...
  if (existing !== undefined) {
    ...
    if (cap.pageTitle !== undefined && cap.pageTitle !== '') existing.pageTitle = cap.pageTitle;
  ...
  if (cap.type === 'ts') {
    ...
    if (cap.groupKey !== undefined && cap.groupKey !== null && cap.groupKey !== '') {
      ...
      if (grouped.segmentUrls === undefined) grouped.segmentUrls = [];
  ...
  if (entry.type === 'dash' || entry.type === 'hls') {
    const pending = state.entries.filter(
      (e) => e !== entry && e.tabId === entry.tabId && e.type === 'stream' && (e.ext === 'm4s' || e.ext === 'mp4'),
    );
    if (pending.length > 0) {
