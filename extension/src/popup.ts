import { bvidFromPageUrl, bvidFromPlayurl, cleanBiliTitle, NATIVE_HOST_NAME, STORAGE_KEY, type Entry, type MediaType } from './lib/sniff.js';
...
async function enrichTitles(entries: Entry[]): Promise<Entry[]> {
  try {
    const tabs = await chrome.tabs.query({ active: true, currentWindow: true });
    const tab = tabs[0];
    if (tab === undefined || tab.url === undefined) return entries;
...
// 诊断信息：帮助在真实站点上定位问题
async function updateDebugInfo(entries: Entry[]): Promise<void> {
  try {
    const tabs = await chrome.tabs.query({ active: true, currentWindow: true });
    const tab = tabs[0];
    if (tab === undefined) {
